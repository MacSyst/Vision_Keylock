...
RegisterKeyMapping("gkey", "Gang Toggle Lock/Unlock", "keyboard", Vision.GangKey)


RegisterCommand("gkey", function(source, rawCommand)
    ...
    if vehicle == 0 then
        ...
        if Vision.okoknotify then
            exports['okokNotify']:Alert("Keylock", "No vehicle found!", Time, 'warning')
        end
        return
    end
    ...
    ESX.TriggerServerCallback('esx:getPlayerData', function(playerData)
        ...
        local allowedJob = Vision.Gang[plate]

        if allowedJob and playerJob == allowedJob then
            ...
            if GetVehicleDoorLockStatus(vehicle) == 1 then
                ...
                if Vision.okoknotify then
                    exports['okokNotify']:Alert("Keylock", "Car is now locked!", Time, 'error')
                end
            ...
            elseif GetVehicleDoorLockStatus(vehicle) == 2 then
                ...
                if Vision.okoknotify then
                    exports['okokNotify']:Alert("Keylock", "Car is now unlocked!", Time, 'success')
                end
